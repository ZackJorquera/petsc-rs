...
use crate::{
    ...
    ksp::KSP,
    mat::{Mat, MatType},
    petsc_panic, petsc_raw,
    snes::{DomainOrPetscError, SNES},
    spaces::{DualSpace, Space},
    vector::{self, Vector},
    InsertMode, Petsc, PetscAsRaw, PetscErrorKind, PetscInt, PetscObject, PetscObjectPrivate,
    PetscReal, PetscScalar, Result,
};
use core::slice;
...
pub struct DM<'a, 'tl> {
    ...
    ksp_compute_operators_trampoline_data:
        Option<Pin<Box<KSPComputeOperatorsTrampolineData<'a, 'tl>>>>,
        ...
        match self {
            ...
            FieldDiscPriv::Unknown(ufd) => {
                ...
                if id == unsafe { petsc_raw::PETSCFE_CLASSID } {
                    ...
                    petsc_panic!(
                        ufd.world,
                        PetscErrorKind::PETSC_ERR_ARG_WRONG,
...
struct DMProjectFunctionTrampolineData<'tl> {
    user_f: Box<
        dyn FnMut(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()>
            + 'tl,
    >,
}
...
type BCFuncDyn<'tl> =
    dyn Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl;
// Because the C method that uses this method doesn't have a context, we cant use this
type BCFieldDyn<'tl> = dyn Fn(
        PetscInt,
        PetscInt,
        PetscInt,
        &[PetscInt],
...
struct KSPComputeOperatorsTrampolineData<'a, 'tl> {
    world: &'a UserCommunicator,
    user_f:
        Box<dyn FnMut(&KSP<'a, '_, '_>, &mut Mat<'a, '_>, &mut Mat<'a, '_>) -> Result<()> + 'tl>,
}
...
impl<'a, 'tl> DM<'a, 'tl> {
    ...
    pub fn set_mat_type(&mut self, mat_type: MatType) -> Result<()> {
        let type_name_cs =
            ::std::ffi::CString::new(mat_type.to_string()).expect("`CString::new` failed");
    ...
    pub fn da_create_1d<'ll>(
        world: &'a UserCommunicator,
        bx: DMBoundaryType,
        nx: PetscInt,
        dof: PetscInt,
        s: PetscInt,
        lx: impl Into<Option<&'ll [PetscInt]>>,
    ) -> Result<Self> {
        let lx = lx.into();
        assert!(lx.map_or(true, |lx| lx.len() == world.size() as usize));
        let mut dm_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::DMDACreate1d(
                world.as_raw(),
                bx,
                nx,
                dof,
                s,
                lx.map_or(std::ptr::null(), |lx| lx.as_ptr()),
                dm_p.as_mut_ptr(),
            )
        };
        unsafe { chkerrq!(world, ierr) }?;
    ...
    pub fn da_create_2d<'ll1, 'll2>(
        ...
        nx: PetscInt,
        ny: PetscInt,
        px: impl Into<Option<PetscInt>>,
        py: impl Into<Option<PetscInt>>,
        dof: PetscInt,
        s: PetscInt,
        lx: impl Into<Option<&'ll1 [PetscInt]>>,
        ly: impl Into<Option<&'ll2 [PetscInt]>>,
    ) -> Result<Self> {
        let lx = lx.into();
        let ly = ly.into();
        ...
        let ierr = unsafe {
            petsc_raw::DMDACreate2d(
                ...
                s,
                lx.map_or(std::ptr::null(), |lx| lx.as_ptr()),
                ly.map_or(std::ptr::null(), |ly| ly.as_ptr()),
                dm_p.as_mut_ptr(),
            )
        };
        unsafe { chkerrq!(world, ierr) }?;
    ...
    pub fn da_create_3d<'ll1, 'll2, 'll3>(
        world: &'a UserCommunicator,
        bx: DMBoundaryType,
        by: DMBoundaryType,
        bz: DMBoundaryType,
        ...
        px: impl Into<Option<PetscInt>>,
        py: impl Into<Option<PetscInt>>,
        pz: impl Into<Option<PetscInt>>,
        dof: PetscInt,
        s: PetscInt,
        lx: impl Into<Option<&'ll1 [PetscInt]>>,
        ly: impl Into<Option<&'ll2 [PetscInt]>>,
        lz: impl Into<Option<&'ll3 [PetscInt]>>,
    ) -> Result<Self> {
        let lx = lx.into();
        let ly = ly.into();
        let lz = lz.into();
        ...
        let ierr = unsafe {
            petsc_raw::DMDACreate3d(
                ...
                lx.map_or(std::ptr::null(), |lx| lx.as_ptr()),
                ly.map_or(std::ptr::null(), |ly| ly.as_ptr()),
                lz.map_or(std::ptr::null(), |lz| lz.as_ptr()),
                dm_p.as_mut_ptr(),
            )
        };
        unsafe { chkerrq!(world, ierr) }?;
    ...
    pub fn plex_create_box_mesh(
        ...
        let periodicity = periodicity
            .into()
            .map(|periodicity| vec![periodicity.0, periodicity.1, periodicity.2]);

        let mut dm_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::DMPlexCreateBoxMesh(
                world.as_raw(),
                dim,
                simplex.into(),
                faces.as_ref().map_or(std::ptr::null(), |f| f.as_ptr()),
                lower.as_ref().map_or(std::ptr::null(), |l| l.as_ptr()),
                upper.as_ref().map_or(std::ptr::null(), |u| u.as_ptr()),
                periodicity
                    .as_ref()
    ...
    pub fn plex_create_from_file(
        world: &'a UserCommunicator,
        file_name: impl AsRef<Path>,
        interpolate: bool,
    ) -> Result<Self> {
        let filename_cs = CString::new(file_name.as_ref().to_str().expect("`Path::to_str` failed"))
            .expect("`CString::new` failed");
        let mut dm_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::DMPlexCreateFromFile(
                world.as_raw(),
                filename_cs.as_ptr(),
                interpolate.into(),
                dm_p.as_mut_ptr(),
    ...
    pub fn get_local_vector(&self) -> Result<vector::BorrowVectorMut<'a, '_>> {
        ...
        Ok(vector::BorrowVectorMut::new(
            vec,
            Some(Box::new(move |borrow_vec| {
                let ierr = unsafe {
                    petsc_raw::DMRestoreLocalVector(self.dm_p, &mut borrow_vec.vec_p as *mut _)
                };
    ...
    pub fn composite_create_local_vectors(&self) -> Result<Vec<Vector<'a>>> {
        if let Some(c) = self.composite_dms.as_ref() {
            c.iter()
                .fold(Ok(Vec::with_capacity(c.len())), |acc, dm| match acc {
    ...
    pub fn global_to_local(
        ...
        let ierr =
            unsafe { petsc_raw::DMGlobalToLocalBegin(self.dm_p, global.vec_p, mode, local.vec_p) };
        unsafe { chkerrq!(self.world, ierr) }?;
        let ierr =
            unsafe { petsc_raw::DMGlobalToLocalEnd(self.dm_p, global.vec_p, mode, local.vec_p) };
        unsafe { chkerrq!(self.world, ierr) }
    }
    ...
    pub fn local_to_global(
        ...
        let ierr =
            unsafe { petsc_raw::DMLocalToGlobalBegin(self.dm_p, local.vec_p, mode, global.vec_p) };
        unsafe { chkerrq!(self.world, ierr) }?;
        let ierr =
            unsafe { petsc_raw::DMLocalToGlobalEnd(self.dm_p, local.vec_p, mode, global.vec_p) };
        unsafe { chkerrq!(self.world, ierr) }
    }
    ...
    /// assert_eq!(
    ///     g_view.view().reversed_axes().as_slice().unwrap(),
    ///     &[
    ///         c(0.0),
    ///         c(1.0),
    ///         c(2.0),
    ...
    ///     assert_eq!(
    ///         g_view.slice(s![.., ..]),
    ///         rhs_array.slice(s![xs..(xs + xm), ys..(ys + ym)])
    ///     );
    /// } else {
    ///     Petsc::set_error(
    ///         petsc.world(),
    ...
    ///     assert_eq!(
    ///         g_view.slice(s![.., ..]),
    ///         l_view.slice(s![xs - gxs..xm, ys - gys..ym])
    ///     );
    /// } else {
    ///     Petsc::set_error(
    ///         petsc.world(),
    ...
    /// # }
    /// ```
    pub fn da_vec_view<'at, 'b>(
        &self,
        vec: &'b Vector<'at>,
    ) -> Result<crate::vector::VectorView<'at, 'b>> {
        ...
        if local_size != gxm * gym * gzm * dof {
            seterrq!(
                self.world,
                PetscErrorKind::PETSC_ERR_ARG_INCOMP,
                format!(
                    "Vector local size {} is not compatible with DMDA local sizes {} or {}\n",
                    local_size,
                    xm * ym * zm * dof,
                    gxm * gym * gzm * dof
                )
        ...
        if dim > 3 || dim < 1 {
            seterrq!(
                self.world,
                PetscErrorKind::PETSC_ERR_ARG_CORRUPT,
        ...
        let ndarray = unsafe {
            ArrayView::from_shape_ptr(
                ndarray::IxDyn(&dims_r[(3 - dim as usize)..]),
                array.assume_init(),
            )
            .reversed_axes()
        };

        Ok(crate::vector::VectorView {
            vec,
    ...
    ///     g_view.slice(s![.., ..]).dim(),
    ///     rhs_array
    ///         .slice(s![gxs..(gxs + gxm), gys..(gys + gym)])
    ///         .dim()
    /// );
    /// assert_eq!(
    ///     g_view.slice(s![.., ..]),
    ///     rhs_array.slice(s![gxs..(gxs + gxm), gys..(gys + gym)])
    /// );
    /// # Ok(())
    /// # }
    /// ```
    pub fn da_vec_view_mut<'at, 'b>(
        &self,
        vec: &'b mut Vector<'at>,
    ) -> Result<crate::vector::VectorViewMut<'at, 'b>> {
    ...
    pub fn da_get_ownership_ranges(&self) -> Result<(Vec<PetscInt>, Vec<PetscInt>, Vec<PetscInt>)> {
        ...
        let ierr = unsafe {
            petsc_raw::DMDAGetOwnershipRanges(
                self.dm_p,
                lx.as_mut_ptr(),
                if dim >= 2 {
                    ly.as_mut_ptr()
        ...
        let lx_vec = {
            let lx_slice =
                unsafe { std::slice::from_raw_parts(lx.assume_init(), self.world.size() as usize) };
            lx_slice.to_vec()
        };
        let ly_vec = if dim >= 2 {
            let ly_slice =
                unsafe { std::slice::from_raw_parts(ly.assume_init(), self.world.size() as usize) };
            ly_slice.to_vec()
        } else {
            vec![]
        };
        let lz_vec = if dim >= 2 {
            let lz_slice =
                unsafe { std::slice::from_raw_parts(lz.assume_init(), self.world.size() as usize) };
            lz_slice.to_vec()
        } else {
            vec![]
        };
        ...
        I: IntoIterator<Item = &'v mut Vector<'a>>,
    {
        if let Some(c) = self.composite_dms.as_ref() {
            ...
            let ierr = unsafe {
                petsc_raw::DMCompositeScatterArray(self.dm_p, gvec.vec_p, lvecs_p.as_mut_ptr())
            };
            unsafe { chkerrq!(self.world, ierr) }
        } else {
            seterrq!(
    ...
    pub fn composite_get_access_mut<'bv>(
        &'bv self,
        gvec: &'bv mut Vector<'a>,
    ) -> Result<Vec<vector::BorrowVectorMut<'a, 'bv>>> {
        ...
        if let Some(c) = self.composite_dms.as_ref() {
            ...
            let ierr = unsafe {
                petsc_raw::DMCompositeGetAccessArray(
                    self.dm_p,
                    gvec.vec_p,
                    c.len() as PetscInt,
                    wanted.as_ptr(),
            ...
            Ok(vec_ps
                ...
                .map(move |(v_p, i)| {
                    ...
                    vector::BorrowVectorMut::new(
                        vec,
                        Some(Box::new(move |borrow_vec| {
                            let i = i;
                            let ierr = unsafe {
                                petsc_raw::DMCompositeRestoreAccessArray(
                                    self.dm_p,
                                    gvec_rc.vec_p,
                                    1,
                                    std::slice::from_ref(&i).as_ptr(),
    ...
    pub fn composite_get_global_indexsets(&self) -> Result<Vec<IS<'a>>> {
        if let Some(c) = self.composite_dms.as_ref() {
            ...
            let len = c.len();
            let ierr =
                unsafe { petsc_raw::DMCompositeGetGlobalISs(self.dm_p, is_array_p.as_mut_ptr()) };
            unsafe { chkerrq!(self.world, ierr) }?;

            let is_slice = unsafe { slice::from_raw_parts(is_array_p.assume_init(), len) };

            let ret_vec = is_slice
                .iter()
                .map(|is_p| IS {
                    world: self.world,
                    is_p: *is_p,
            ...
            let cs_fn_name =
                CString::new("composite_get_global_indexsets").expect("CString::new failed");
            let cs_file_name = CString::new("dm.rs").expect("CString::new failed");

            // Note, `PetscFree` is a macro around `PetscTrFree`
            let ierr = unsafe {
                (petsc_raw::PetscTrFree.unwrap())(
                    is_array_p.assume_init() as *mut _,
                    line!() as i32,
                    cs_fn_name.as_ptr(),
                    cs_file_name.as_ptr(),
                )
    ...
    pub fn composite_get_local_indexsets(&self) -> Result<Vec<IS<'a>>> {
        if let Some(c) = self.composite_dms.as_ref() {
            ...
            let len = c.len();
            let ierr =
                unsafe { petsc_raw::DMCompositeGetLocalISs(self.dm_p, is_array_p.as_mut_ptr()) };
            unsafe { chkerrq!(self.world, ierr) }?;
    ...
    pub fn add_field(
        &mut self,
        label: impl Into<Option<DMLabel<'a>>>,
        field: impl Into<FieldDisc<'a, 'tl>>,
    ) -> Result<()> {
        ...
        if is_correct_type {
            ...
            let ierr = unsafe {
                petsc_raw::DMAddField(
                    self.dm_p,
                    label.as_ref().map_or(std::ptr::null_mut(), |l| l.dml_p),
                    field.as_raw(),
                )
            };
            unsafe { chkerrq!(self.world, ierr) }?;
    ...
    pub fn get_label(&self, name: &str) -> Result<Option<DMLabel<'a>>> {
        ...
        let ierr =
            unsafe { petsc_raw::DMGetLabel(self.dm_p, name_cs.as_ptr(), dm_label.as_mut_ptr()) };
        unsafe { chkerrq!(self.world, ierr) }?;

        let dm_label = NonNull::new(unsafe { dm_label.assume_init() });

        let mut label = dm_label.map(|nn_dml_p| DMLabel {
            world: self.world,
            dml_p: nn_dml_p.as_ptr(),
        });
    ...
    unsafe fn complete_boundary_label_internal(
        &self,
        ds: &DS,
        field: PetscInt,
        bd_num: PetscInt,
        ...
        if let Some(ref fields) = self.fields {
            ...
            if id == petsc_raw::PETSCFE_CLASSID {
                ...
                if !duplicate {
                    ...
                    let ierr = petsc_raw::DMConvert(
                        self.dm_p,
                        petsc_raw::DMPLEX.as_ptr() as *const _,
                        plex_p.as_mut_ptr(),
                    );
    ...
    pub fn add_boundary_essential<F1>(
        &mut self,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        ...
        F1: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        unsafe {
            self.complete_boundary_label_internal(ds, field, PetscInt::MAX, label)?;
        }
        self.get_ds_or_create()?.add_boundary_essential(
            name,
            label,
            values,
            field,
    ...
    /// * `bcval` *(output)* - output values at the current point
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn add_boundary_essential_with_dt<F1, F2>(
        &mut self,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        ...
        F2: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        self.get_ds_or_create()?.add_boundary_essential_with_dt(
            name,
            label,
            values,
            field,
        ...
        F1: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        let ds = self.try_get_ds().unwrap();
        unsafe {
            self.complete_boundary_label_internal(ds, field, PetscInt::MAX, labelname)?;
        }
        self.get_ds_or_create()?.add_boundary_essential(
            name,
            labelname,
    ...
    /// Look at [`snes-ex12`](https://gitlab.com/petsc/petsc-rs/-/blob/main/examples/snes/src/ex12.rs)
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn add_boundary_natural<F1>(
        &mut self,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        ...
        F1: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        self.get_ds_or_create()?.add_boundary_natural(
            name,
            label,
            values,
            field,
    ...
    /// * `bc_user_func_t` - A pointwise function giving the time deriative of the boundary values
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn add_boundary_natural_with_dt<F1, F2>(
        &mut self,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        ...
        F2: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        self.get_ds_or_create()?.add_boundary_natural_with_dt(
            name,
            label,
            values,
            field,
    ...
    /// * `bcval` *(output)* - output values at the current point
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn add_boundary_field_raw(
        &mut self,
        bctype: DMBoundaryConditionType,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        field: PetscInt,
        comps: &[PetscInt],
        bc_user_func: impl Into<
            Option<
                unsafe extern "C" fn(
                    PetscInt,
                    PetscInt,
        ...
        self.get_ds_or_create()?.add_boundary_field_raw(
            bctype,
            name,
            label,
            values,
    ...
    /// * `bcval` *(output)* - output values at the current point
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn add_boundary_essential_bd_field_raw(
        &mut self,
        name: &str,
        label: &mut DMLabel,
        values: &[PetscInt],
        ...
        self.get_ds_or_create()?
            .add_boundary_essential_bd_field_raw(
                name,
                label,
                values,
                field,
    ...
    pub fn create_ds(&mut self) -> Result<()> {
        ...
        unsafe {
            self.ds.as_mut().unwrap().reference()?;
        }
    ...
    pub fn get_periodicity(
        &self,
    ) -> Result<Option<(&[PetscReal], &[PetscReal], &[DMBoundaryType])>> {
    ...
    pub fn project_function_local<'sl>(
        &self,
        time: PetscReal,
        mode: InsertMode,
        local: &mut Vector,
        user_funcs: impl IntoIterator<
            Item = Box<
                dyn FnMut(
                        PetscInt,
                        PetscReal,
        ...
        let trampoline_datas = user_funcs
            .into_iter()
            .map(|closure_anchor| {
                Box::pin(DMProjectFunctionTrampolineData {
        ...
        if trampoline_datas.len() != nf {
            seterrq!(
                self.world,
                PetscErrorKind::PETSC_ERR_USER_INPUT,
                format!(
                    "Expected {} functions in `user_funcs`, but there were {}.",
                    nf,
                    trampoline_datas.len()
                )
            )?;
        }

        unsafe extern "C" fn dm_project_function_local_trampoline(
            dim: PetscInt,
            time: PetscReal,
            x: *const PetscReal,
            nf: PetscInt,
            u: *mut petsc_raw::PetscScalar,
            ctx: *mut ::std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
            let trampoline_data: Pin<&mut DMProjectFunctionTrampolineData> =
        ...
        let mut trampoline_funcs: Vec<
            ::std::option::Option<
                unsafe extern "C" fn(
                    ...
                    arg4: PetscInt,
                    arg5: *mut petsc_raw::PetscScalar,
                    arg6: *mut ::std::os::raw::c_void,
                ) -> petsc_raw::PetscErrorCode,
            >,
        > = vec![Some(dm_project_function_local_trampoline); nf];
        let mut trampoline_data_refs = trampoline_datas
            .iter()
            .map(|td| unsafe { std::mem::transmute(td.as_ref()) })
            .collect::<Vec<_>>();

        let ierr = unsafe {
            petsc_raw::DMProjectFunctionLocal(
                self.dm_p,
                time,
                trampoline_funcs.as_mut_ptr(),
    ...
    pub fn project_field_local_raw<'vl>(
        &mut self,
        time: PetscReal,
        localu: impl Into<Option<&'vl Vector<'a>>>,
        mode: InsertMode,
        localx: &mut Vector,
        user_funcs: impl IntoIterator<
            Item = Option<
        ...
        'a: 'vl,
    {
        ...
        let mut fn_ptrs = user_funcs.into_iter().collect::<Vec<
            Option<
                unsafe extern "C" fn(
                    PetscInt,
                    PetscInt,
        ...
        let localu = if let Some(localu) = localu.into() {
            ...
            unsafe {
                localu_owned.reference()?;
            }
            &localu_owned
        };

        let ierr = unsafe {
            petsc_raw::DMProjectFieldLocal(
                self.dm_p,
                time,
                localu.vec_p,
                fn_ptrs.as_mut_ptr() as *mut _,
                mode,
                localx.vec_p,
            )
        };
    ...
    pub fn project_field_raw<'vl>(
        &mut self,
        time: PetscReal,
        u: impl Into<Option<&'vl Vector<'a>>>,
        mode: InsertMode,
        x: &mut Vector,
        user_funcs: impl IntoIterator<
            Item = Option<
    ...
    pub fn compute_l2_diff<'sl>(
        ...
        unsafe extern "C" fn dm_compute_ls_diff_trampoline(
            dim: PetscInt,
            time: PetscReal,
            x: *const PetscReal,
            nf: PetscInt,
    ...
    pub fn plex_compute_bd_integral_raw<'vll, 'll, 'lal: 'll>(
        &mut self,
        x: &Vector,
        label: impl Into<Option<&'ll DMLabel<'lal>>>,
        vals: impl Into<Option<&'vll [PetscInt]>>,
        ...
        let ierr = unsafe {
            petsc_raw::DMPlexComputeBdIntegral(
                self.dm_p,
                x.vec_p,
                label.into().as_raw(),
                vals.as_ref()
                    .map_or(petsc_raw::PETSC_DETERMINE, |&vl| vl.len() as PetscInt),
    ...
    pub fn has_label(&self, labelname: &str) -> Result<bool> {
        ...
        let ierr =
            unsafe { petsc_raw::DMHasLabel(self.dm_p, labelname_cs.as_ptr(), res.as_mut_ptr()) };
        unsafe { chkerrq!(self.world, ierr) }?;
    ...
    pub fn plex_mark_boundary_faces(
        &mut self,
        val: impl Into<Option<PetscInt>>,
        label: &mut DMLabel,
    ) -> Result<()> {
        let ierr = unsafe {
            petsc_raw::DMPlexMarkBoundaryFaces(
                self.dm_p,
                val.into().unwrap_or(petsc_raw::PETSC_DETERMINE),
                label.dml_p,
            )
        };
        unsafe { chkerrq!(self.world, ierr) }
    ...
    /// Internal function
    fn get_field_from_c_struct(
        &self,
        f: PetscInt,
    ) -> Result<(Option<DMLabel<'a>>, FieldDiscPriv<'a, 'tl>)> {
        let mut dml_p = MaybeUninit::uninit();
        let mut f_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::DMGetField(self.dm_p, f, dml_p.as_mut_ptr(), f_p.as_mut_ptr() as *mut _)
        };
        unsafe { chkerrq!(self.world, ierr) }?;
        ...
        let mut field = crate::PetscObjectStruct {
            world: self.world,
            po_p: unsafe { f_p.assume_init() },
        };
        unsafe {
            field.reference()?;
        }
    ...
    #[cfg(any(petsc_version_3_16_dev, doc))]
    pub fn get_auxiliary_vec<'ll, 'lal: 'll>(
        &self,
        label: impl Into<Option<&'ll DMLabel<'lal>>>,
        value: PetscInt,
    ) -> Result<Option<Rc<Vector<'a>>>> {
        let mut vec_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::DMGetAuxiliaryVec(
                self.dm_p,
                label.into().as_raw(),
                value,
                vec_p.as_mut_ptr(),
        ...
        let mut vec = vec_p_nn.map(|nn_vec_p| Vector {
            world: self.world,
            vec_p: nn_vec_p.as_ptr(),
        });
    ...
    pub fn get_label_id_is(&self, name: &str) -> Result<Option<IS<'a>>> {
        ...
        let ierr =
            unsafe { petsc_raw::DMGetLabelIdIS(self.dm_p, name_cs.as_ptr(), is_p.as_mut_ptr()) };
        unsafe { chkerrq!(self.world, ierr) }?;

        let nn_is_p = NonNull::new(unsafe { is_p.assume_init() });
        Ok(nn_is_p.map(|nn_is_p| IS {
            world: self.world,
            is_p: nn_is_p.as_ptr(),
        }))
    }
            ...
            + 'tl,
    {
        ...
        unsafe extern "C" fn snes_function_trampoline(
            snes_p: *mut petsc_raw::_p_SNES,
            x_p: *mut petsc_raw::_p_Vec,
            f_p: *mut petsc_raw::_p_Vec,
            ctx: *mut std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
            ...
            let x = ManuallyDrop::new(Vector {
                world: trampoline_data.world,
                vec_p: x_p,
            });
            let mut f = ManuallyDrop::new(Vector {
                world: trampoline_data.world,
                vec_p: f_p,
            });

            (trampoline_data.get_mut().user_f)(&snes, &x, &mut f).map_or_else(
                |err| match err {
        ...
        let ierr = unsafe {
            petsc_raw::DMSNESSetFunction(
                self.dm_p,
                Some(snes_function_trampoline),
            ...
            + 'tl,
    {
        ...
        unsafe extern "C" fn snes_jacobian_single_trampoline(
            snes_p: *mut petsc_raw::_p_SNES,
            vec_p: *mut petsc_raw::_p_Vec,
            mat1_p: *mut petsc_raw::_p_Mat,
            mat2_p: *mut petsc_raw::_p_Mat,
            ctx: *mut ::std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
            ...
            let trampoline_data: Pin<&mut SNESJacobianSingleTrampolineData> =
                std::mem::transmute(ctx);
            ...
            (trampoline_data.get_mut().user_f)(&snes, &x, &mut a_mat).map_or_else(
                |err| match err {
                    DomainOrPetscError::DomainErr => {
        ...
        let ierr = unsafe {
            petsc_raw::DMSNESSetJacobian(
                self.dm_p,
                Some(snes_jacobian_single_trampoline),
                std::mem::transmute(trampoline_data.as_ref()),
            )
        };
        unsafe { chkerrq!(self.world, ierr) }?;

        self.snes_jacobian_trampoline_data =
            Some(SNESJacobianTrampolineData::SingleMat(trampoline_data));
            ...
            + 'tl,
    {
        ...
        unsafe extern "C" fn snes_jacobian_double_trampoline(
            snes_p: *mut petsc_raw::_p_SNES,
            vec_p: *mut petsc_raw::_p_Vec,
            mat1_p: *mut petsc_raw::_p_Mat,
            mat2_p: *mut petsc_raw::_p_Mat,
            ...
            // SAFETY: read `snes_function_trampoline` safety
            let trampoline_data: Pin<&mut SNESJacobianDoubleTrampolineData> =
                std::mem::transmute(ctx);
            ...
            (trampoline_data.get_mut().user_f)(&snes, &x, &mut a_mat, &mut p_mat).map_or_else(
                |err| match err {
                    DomainOrPetscError::DomainErr => {
                        let perr = snes.set_jacobian_domain_error();
                        match perr {
        ...
        let ierr = unsafe {
            petsc_raw::DMSNESSetJacobian(
                self.dm_p,
                Some(snes_jacobian_double_trampoline),
                std::mem::transmute(trampoline_data.as_ref()),
            )
        };
        unsafe { chkerrq!(self.world, ierr) }?;
        ...
        self.snes_jacobian_trampoline_data =
            Some(SNESJacobianTrampolineData::DoubleMat(trampoline_data));
        ...
        F: FnMut(&KSP<'a, '_, '_>, &mut Mat<'a, '_>, &mut Mat<'a, '_>) -> Result<()> + 'tl,
    {
        ...
        unsafe extern "C" fn ksp_compute_operators_trampoline(
            ksp_p: *mut petsc_raw::_p_KSP,
            mat1_p: *mut petsc_raw::_p_Mat,
            mat2_p: *mut petsc_raw::_p_Mat,
            ctx: *mut std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
            ...
            // called.
            // We don't construct a Box<> because we dont want to drop anything
            let trampoline_data: Pin<&mut KSPComputeOperatorsTrampolineData> =
                std::mem::transmute(ctx);
        ...
        let ierr = unsafe {
            petsc_raw::DMKSPSetComputeOperators(
                self.dm_p,
                Some(ksp_compute_operators_trampoline),
                std::mem::transmute(trampoline_data.as_ref()),
            )
        }; // this will also erase the lifetimes
        unsafe { chkerrq!(self.world, ierr) }?;
        ...
        F: FnMut(&KSP<'a, '_, '_>, &mut Vector<'a>) -> Result<()> + 'tl,
    {
        ...
        unsafe extern "C" fn ksp_compute_rhs_trampoline(
            ksp_p: *mut petsc_raw::_p_KSP,
            vec_p: *mut petsc_raw::_p_Vec,
            ctx: *mut std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
            ...
            let mut vec = ManuallyDrop::new(Vector {
                world: trampoline_data.world,
                vec_p,
            });
        ...
        let ierr = unsafe {
            petsc_raw::DMKSPSetComputeRHS(
                self.dm_p,
                Some(ksp_compute_rhs_trampoline),
                std::mem::transmute(trampoline_data.as_ref()),
            )
        }; // this will also erase the lifetimes
        unsafe { chkerrq!(self.world, ierr) }?;
...
impl<'a> FEDisc<'a, '_> {
    ...
    pub fn create_default<'pl>(
        world: &'a UserCommunicator,
        dim: PetscInt,
        nc: PetscInt,
        is_simplex: bool,
        prefix: impl Into<Option<&'pl str>>,
        qorder: impl Into<Option<PetscInt>>,
    ) -> Result<Self> {
        let cstring = prefix
            .into()
            .map(|p| CString::new(p).expect("`CString::new` failed"));
        let mut fe_p = MaybeUninit::uninit();
        let ierr = unsafe {
            petsc_raw::PetscFECreateDefault(
                world.as_raw(),
                dim,
                nc,
                is_simplex.into(),
                cstring.as_ref().map_or(std::ptr::null(), |p| p.as_ptr()),
                qorder.into().unwrap_or(petsc_raw::PETSC_DETERMINE),
                fe_p.as_mut_ptr(),
            )
        };
        unsafe { chkerrq!(world, ierr) }?;
...
impl<'a, 'tl> DS<'a, 'tl> {
    ...
    pub fn get_boundary_info(
        ...
        let mut wf = WeakForm {
            world: self.world,
            wf_p: unsafe { wf_p.assume_init() },
        };
        unsafe {
            wf.reference()?;
        }
        let mut label = DMLabel {
            world: self.world,
            dml_p: unsafe { dml_p.assume_init() },
        };
        unsafe {
            label.reference()?;
        }
        let name = unsafe { CStr::from_ptr(name_cs.assume_init()) }
            .to_str()
            .unwrap();
        let values =
            unsafe { slice::from_raw_parts(values_p.assume_init(), nv.assume_init() as usize) };
        let comps =
            unsafe { slice::from_raw_parts(comps_p.assume_init(), nc.assume_init() as usize) };

        Ok((
            wf,
            unsafe { bct.assume_init() },
            name,
            label,
            values,
    ...
    pub fn get_boundary_info(
        ...
        let name = unsafe { CStr::from_ptr(name_cs.assume_init()) }
            .to_str()
            .unwrap();
        let label = unsafe { CStr::from_ptr(label_cs.assume_init()) }
            .to_str()
            .unwrap();
        let comps =
            unsafe { slice::from_raw_parts(comps_p.assume_init(), nc.assume_init() as usize) };
        let ids =
            unsafe { slice::from_raw_parts(ids_p.assume_init(), nids.assume_init() as usize) };
    ...
    pub fn set_constants(&mut self, consts: &[PetscInt]) -> Result<()> {
        ...
        let ierr = unsafe {
            petsc_raw::PetscDSSetConstants(
                self.ds_p,
                consts.len() as PetscInt,
                consts.as_ptr() as *mut _,
            )
        };
        unsafe { chkerrq!(self.world, ierr) }
    }
        ...
        F: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        unsafe extern "C" fn ds_exact_solution_trampoline(
            dim: PetscInt,
            time: PetscReal,
            x: *const PetscReal,
            nc: PetscInt,
        ...
        let ierr = unsafe {
            petsc_raw::PetscDSSetExactSolution(
                self.ds_p,
                f,
                Some(ds_exact_solution_trampoline),
                std::mem::transmute(trampoline_data.as_ref()),
            )
        };
        unsafe { chkerrq!(self.world, ierr) }?;
        ...
        F1: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
    {
        ...
        let raw_fn_wrapper = |ds_p, name, nc, comps, field, fn1, fn2, ctx| unsafe {
            petsc_raw::PetscDSAddBoundary(
                ds_p,
                DMBoundaryConditionType::DM_BC_ESSENTIAL,
                name,
                label.dml_p,
                values.len() as PetscInt,
                values.as_ptr(),
                field,
                nc,
                comps,
        ...
        self.add_boundary_func_shared(
            name,
            field,
            comps,
            bc_user_func,
            Option::<
                fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()>,
            >::None,
            raw_fn_wrapper,
        )?;
    ...
    fn add_boundary_func_shared<F1, F2, F3>(
        &mut self,
        name: &str,
        field: PetscInt,
        comps: &[PetscInt],
        bc_user_func: F1,
        bc_user_func_t: Option<F2>,
        add_boundary_wrapper: F3,
    ) -> Result<()>
    where
        F1: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
        F2: Fn(PetscInt, PetscReal, &[PetscReal], PetscInt, &mut [PetscScalar]) -> Result<()> + 'tl,
        F3: FnOnce(
            *mut petsc_raw::_p_PetscDS,
        ...
        ) -> petsc_raw::PetscErrorCode,
    {
        ...
        let closure_anchor2 = bc_user_func_t
            .map(|bc_user_func_t| -> Box<BCFuncDyn<'tl>> { Box::new(bc_user_func_t) });

        let trampoline_data = Box::pin(DMBoundaryFuncTrampolineData {
            user_f1: closure_anchor1,
            user_f2: closure_anchor2,
        });

        unsafe extern "C" fn bc_func_trampoline(
            dim: PetscInt,
            time: PetscReal,
            x: *const PetscReal,
            nc: PetscInt,
            bcval: *mut petsc_raw::PetscScalar,
            ctx: *mut ::std::os::raw::c_void,
        ) -> petsc_raw::PetscErrorCode {
        ...
        unsafe extern "C" fn bc_func_t_trampoline(
            dim: PetscInt,
            time: PetscReal,
            x: *const PetscReal,
            nc: PetscInt,
            ...
            (trampoline_data.get_mut().user_f2.as_mut().unwrap())(
                dim,
                time,
                x_slice,
                nc,
        ...
        let ierr = add_boundary_wrapper(
            self.ds_p,
            name_cs.as_ptr(),
            comps.len() as PetscInt,
            comps.as_ptr(),
            field,
            unsafe { std::mem::transmute(bc_func_essential_trampoline_fn_ptr) },
        ...
        if let Some(ref mut boundary_trampoline_data_vec) = self.boundary_trampoline_data {
            ...
            self.boundary_trampoline_data =
                Some(vec![DMBoundaryTrampolineData::BCFunc(trampoline_data)]);
        }
    ...
    pub fn add_boundary_field_raw(
        ...
        let raw_fn_wrapper = |ds_p, bctype, name, nc, comps, field, fn1, fn2, ctx| unsafe {
            petsc_raw::PetscDSAddBoundary(
                ds_p,
                bctype,
                name,
        ...
        self.add_boundary_field_raw_shared(
            bctype,
            name,
            field,
            comps,
    ...
    fn add_boundary_field_raw_shared<F3>(
        &mut self,
        bctype: DMBoundaryConditionType,
        name: &str,
        field: PetscInt,
        ...
        ) -> petsc_raw::PetscErrorCode,
    {
        ...
        let ierr = add_boundary_wrapper(
            ...
            comps.as_ptr(),
            field,
            unsafe { std::mem::transmute(bc_user_func) },
