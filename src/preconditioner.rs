...
use crate::{
    indexset::IS, mat::Mat, petsc_raw, vector::Vector, PetscAsRaw, PetscObject, PetscObjectPrivate,
    Result,
};
...
impl<'a, 'tl, 'bl> PC<'a, 'tl, 'bl> {
    ...
    pub fn get_operators_or_create<'rl>(
        ...
        let a_mat = if let Some(a_mat) = self.ref_amat {
            ...
            if self.owned_amat.is_none() {
                ...
                let ierr = unsafe {
                    petsc_raw::PCGetOperators(self.pc_p, a_mat_p.as_mut_ptr(), std::ptr::null_mut())
                };
                unsafe { chkerrq!(self.world, ierr) }?;
        ...
        let p_mat = if let Some(p_mat) = self.ref_pmat {
            ...
            if self.owned_pmat.is_none() {
                ...
                let ierr = unsafe {
                    petsc_raw::PCGetOperators(self.pc_p, std::ptr::null_mut(), p_mat_p.as_mut_ptr())
                };
                unsafe { chkerrq!(self.world, ierr) }?;
        ...
        F: FnMut(&PC<'a, 'tl, '_>, &Vector<'a>, &mut Vector<'a>) -> Result<()> + 'tl,
    {
        ...
        unsafe extern "C" fn pc_shell_set_apply_trampoline(
            pc_p: *mut petsc_raw::_p_PC,
            xin_p: *mut petsc_raw::_p_Vec,
            xout_p: *mut petsc_raw::_p_Vec,
            ...
            let trampoline_data: Pin<&mut PCShellSetApplyTrampolineData> =
            ...
            let xin = ManuallyDrop::new(Vector {
                world: trampoline_data.world,
                vec_p: xin_p,
            });
            let mut xout = ManuallyDrop::new(Vector {
                world: trampoline_data.world,
                vec_p: xout_p,
            });
        ...
        let ierr =
            unsafe { petsc_raw::PCShellSetApply(self.pc_p, Some(pc_shell_set_apply_trampoline)) };
        unsafe { chkerrq!(self.world, ierr) }?;
        let ierr = unsafe {
            petsc_raw::PCShellSetContext(self.pc_p, std::mem::transmute(trampoline_data.as_ref()))
    ...
    pub fn field_split_set_is<'strl>(
        &mut self,
        splitname: impl Into<Option<&'strl str>>,
        is: IS,
    ) -> Result<()> {
        let splitname_cs = splitname
            .into()
            .map(|to_str| CString::new(to_str.to_string()).expect("`CString::new` failed"));

        let ierr = unsafe {
            petsc_raw::PCFieldSplitSetIS(
                self.pc_p,
                splitname_cs
                    .map(|cs| cs.as_ptr())
