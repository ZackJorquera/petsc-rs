...
fn main() -> petsc_rs::Result<()> {
...

    {
        ...
        u_view
            .indexed_iter_mut()
            .map(|(s, u)| ((s[0] + xsu as usize) as PetscReal, u))
            .for_each(|(i, u)| {
                *u = i * hx * (1.0 - i * hx);
            });
        k_view
            .indexed_iter_mut()
            .map(|(s, k)| ((s[0] + xsk as usize) as PetscReal, k))
            .for_each(|(i, k)| {
    ...
    match problem_type {
        ...
        2 => {
            ...
            snes.set_function(Some(&mut f), |snes, x, y| {
                ...
                let dms = snes
                    .try_get_dm()
                    .map(|pack| pack.composite_dms().ok())
                    .flatten()
                    .unwrap_or(dms);
            ...
            snes.set_jacobian_single_mat(&mut b_mat, |snes, x, jac| {
                ...
                if !jac.type_compare(MatType::MATNEST)? {
                    form_jac_uk(
                        &petsc,
                        (mx_u, xm_u, xs_u, xs_k, gxs_u, gxs_k),
                        (&dms[0], &dms[1]),
                        &u_loc,
                        &k_loc,
                    ...
                    form_jac_ku(
                        &petsc,
                        (mx_u, xm_k, xs_k, xs_u, gxs_k, gxs_u),
                        (&dms[0], &dms[1]),
                        &u_loc,
                        &k_loc,
        ...
        _ => {
            Petsc::set_error(
                ...
                format!(
                    "{} is not valid for `-problem_type`. Please use 0, 1, or 2.",
                    problem_type
                ),
            )?;
        }
...
fn form_func_u<'a>(
    _petsc: &Petsc,
    info: (PetscInt, PetscInt, PetscInt, PetscInt, PetscInt),
    dms: (&DM<'a, '_>, &DM<'a, '_>),
    u_loc: &Vector<'a>,
    k_loc: &Vector<'a>,
    y: &mut Vector<'a>,
    ...
    y_view
        .indexed_iter_mut()
        .map(|(s, y)| (s[0], s[0] + (xs - gxs) as usize, y))
        .for_each(|(i, ii, y)| {
            ...
            if i + xs as usize == 0 {
                ...
                *y = hx
                    * ((k_view[ii_k - 1] * (u_view[ii] - u_view[ii - 1])
                        - k_view[ii_k] * (u_view[ii + 1] - u_view[ii]))
                        / (hx * hx)
                        - 1.0)
            }
        });
...
fn form_jac_u<'a>(
    ...
    jac.set_local_values_with_batched(
        (xs..xs + xm).map(|i| {
...
fn form_jac_uk<'a>(
    _petsc: &Petsc,
    info: (PetscInt, PetscInt, PetscInt, PetscInt, PetscInt, PetscInt),
    dms: (&DM<'a, '_>, &DM<'a, '_>),
    u_loc: &Vector<'a>,
    k_loc: &Vector<'a>,
    jac: &mut Mat<'a, '_>,
    ...
    jac.set_local_values_with_batched(
        (xs..xs + xm).map(|i| {
            ...
            (
                ...
                cols,
                vec![
                    -hx * (iw_ubar * ubar_l + iw_gradu * gradu_l),
