...
impl PetscOpt for Opt {
    fn from_petsc_opt_builder(pob: &mut PetscOptBuilder) -> petsc_rs::Result<Self> {
        Ok(Opt {
            ...
            restart: pob.options_bool(
                "-restart",
                "Read in the mesh and solution from a file",
                "snes-ex12",
                false,
            )?,
...
fn create_mesh<'a, 'b>(
    ...
    opt: &Opt,
) -> petsc_rs::Result<(DM<'a, 'b>, Option<Vec<PetscInt>>)> {
        ...
        match opt.variable_coefficient {
            CoeffType::COEFF_NONE => {
                if periodicity.is_some()
                    && periodicity.as_ref().unwrap()[0] != DMBoundaryType::DM_BOUNDARY_NONE
                {
    ...
    match dim {
        2 => {
            ...
            if opt.bc_type == BCType::NEUMANN {
                ...
                let (mut wf, _, _, _, _, _, _) =
                    dm.try_get_ds_mut().unwrap().get_boundary_info(bd)?;
        ...
        _ => {
            ...
            Petsc::set_error(
                dm.world(),
                PetscErrorKind::PETSC_ERR_ARG_OUTOFRANGE,
...
fn setup_aux_dm(
    ...
    if let Some(fe_aux) = fe_aux {
        ...
        {
            let mut this_fe_aux =
                FEDisc::create_default(dm.world(), dim, 1, simplex, "mat_", None)?;
...
fn main() -> petsc_rs::Result<()> {
    ...
    if opt.run_type == RunType::RUN_FULL || opt.run_type == RunType::RUN_EXACT {
        ...
        if opt.run_type == RunType::RUN_FULL {
            snes.get_dm_or_create()?.project_function(
                0.0,
        ...
        let error = snes
            .get_dm_or_create()?
            .compute_l2_diff(0.0, &u, exact_funcs)?;
        if error < tol {
...
unsafe extern "C" fn quadratic_u_field_2d(
    ...
    _nf: PetscInt,
    _nf_aux: PetscInt,
    _u_off: *const PetscInt,
    _u_off_x: *const PetscInt,
    _u: *const PetscScalar,
    _u_t: *const PetscScalar,
    _u_x: *const PetscScalar,
    _a_off: *const PetscInt,
    _a_off_x: *const PetscInt,
    a: *const PetscScalar,
    _a_t: *const PetscScalar,
    _a_x: *const PetscScalar,
    _t: PetscReal,
    _x: *const PetscReal,
    _nc: PetscInt,
    _consts: *const PetscScalar,
    uexact: *mut PetscScalar,
) {
    *uexact = *a;
}
...
fn cross_u_2d(
    ...
    u[0] = PetscScalar::from(
        PetscReal::sin(alpha * xy)
...
unsafe extern "C" fn f0_circle_u(
    ...
    f0[0] = (-4.0 * alpha - 8.0 * alpha.powi(2) * r2 * PetscScalar::tanh(xi))
        * (1.0 / PetscScalar::cosh(xi)).powi(2);
}
...
unsafe extern "C" fn f0_checkerboard_0_u(
    ...
    f0[0] =
        PetscScalar::from(-20.0) * PetscReal::exp(-((x[0] - 0.5).powi(2) + (x[1] - 0.5).powi(2)));
}
...
unsafe extern "C" fn g3_uu(
    ...
    for d in 0..dim as usize {
        g3[d * dim as usize + d] = PetscScalar::from(1.0);
    }
}
...
fn checkerboard_coeff(
    ...
    _nc: PetscInt,
    u: &mut [PetscScalar],
    ctx: (&Opt, &Option<Vec<PetscInt>>),
) -> petsc_rs::Result<()> {
    let (opt, kgrid) = ctx;
    let k;
    let mask = (0..dim as usize).fold(0, |mask, d| {
        (mask + (x[d] * opt.div as PetscReal) as PetscInt) % 2
    });
...
unsafe extern "C" fn g3_analytic_uu(
    ...
    for d in 0..dim as usize {
        g3[d * dim as usize + d] = PetscScalar::from(x[0] + x[1]);
    }
}
